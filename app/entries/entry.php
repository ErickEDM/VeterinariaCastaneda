<?php
	...
	$query = "SELECT `id`,`titulo`,`contenido` FROM `articulo` WHERE id = '$entryID'" ;
	...
	if(isset($_POST["submit"])){
		...
		$query = "UPDATE `articulo` SET `titulo`='$updateTitle',`contenido`='$updateContent' WHERE id = '$entryID'" ;
		$mysqli->query($query);
		echo "<script type='text/javascript'>window.location = 'http://localhost/veterinaria/app/entries/';</script>";
	}

	if(isset($_POST["delete"])){
		$query = "DELETE FROM `articulo` WHERE id = '$entryID'" ;
		$mysqli->query($query);
		echo "<script type='text/javascript'>window.location = 'http://localhost/veterinaria/app/entries/';</script>";
	}
        ...
        <div class="container">
            <div class="row">
                <form class="col-12" method="POST" id="updateEntryForm" enctype="multipart/form-data">
					...
					<input class="formInputField col-12 col-md-9" type="text" id="updateTitle" autocomplete="off" required name="updateTitle" placeholder="Título" max="100" value="<?php if(isset($title)){echo $title;} ?>">
                    <textarea class="formInputField col-12" type="text" id="updateContent" autocomplete="off" required name="updateContent" placeholder="Contenido" ><?php if(isset($content)){echo $content;} ?></textarea>
                    <input class="offset-2 col-8 offset-md-0 col-md-2" type="submit" value="Guardar" id="submit" name="submit" onclick="return confirm('¿Segur@ que desea guardar los cambios?');">
                    <input class="offset-2 col-8 offset-md-1 col-md-2" type="button" value="Cancelar" id="cancel" onclick="cancelEntry()">
                </form>
				<form class="col-12" method="POST" id="deleteEntryForm">
                    <input class="offset-2 col-8 offset-md-10 col-md-2" type="submit" value="Eliminar" id="delete" name="delete" onclick="return confirm('¿Segur@ que desea eliminar la entrada <?php echo $title;?>?');">
                </form>
            </div>
        </div>
